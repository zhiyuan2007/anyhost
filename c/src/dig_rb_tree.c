...
rbtree_create (int (*cmpf)(const void *, const void *))
{
    ...
    DIG_CALLOC(rbtree, 1, sizeof(rbtree_t));
    if (!rbtree)
        return NULL;

	/* Initialize it */
	rbtree->root = RBTREE_NULL;
    rbtree->_node = rbtree->root;
	rbtree->count = 0;
	rbtree->cmp = cmpf;
...
rbtree_insert (rbtree_t *rbtree, rbnode_t *data)
{
	...
	while (node != RBTREE_NULL) {
		...
		if ((r = rbtree->cmp(data->value, node->value)) == 0) {
			return node;
		}
		parent = node;
...
rbtree_delete(rbtree_t *rbtree, const void *key)
{
	rbnode_t *to_delete;
	rbnode_t *child;
	if((to_delete = rbtree_search(rbtree, key)) == 0) return NULL;
	rbtree->count--;
...
void rbtree_delete_node(rbnode_t *node, destroy_func func)
{
    if (node == NULL || node == RBTREE_NULL )
        return;
    if (node->left)
    rbtree_delete_node(node->left, func);
    if (node->right)
    rbtree_delete_node(node->right, func);
    func(node);
}
void rbtree_destory(rbtree_t *rbtree, destroy_func func)
{
    if (func)
    {
        rbtree_delete_node(rbtree->root, func);
    }
