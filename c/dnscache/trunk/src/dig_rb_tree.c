 ...
 */
rbtree_t *
rbtree_create (mem_pool_t *node_pool, int (*cmpf)(const void *, const void *))
{
...
rbtree_insert (rbtree_t *rbtree, rbnode_t *data)
{
	...
	while (node != RBTREE_NULL) {
		...
		if ((r = rbtree->cmp(data->mkey, node->mkey)) == 0) {
			return node;
		}
		parent = node;
