...
{
	ASSERT(!(buffer->_fixed), "fixed buffer can not be reserved\n");
...
buffer_printf(buffer_t *buffer, const char *format, ...)
{
	...
	ASSERT(buffer->_limit == buffer->_reservered_capacity, "read limit is not the same as capacity of a buffer\n");
