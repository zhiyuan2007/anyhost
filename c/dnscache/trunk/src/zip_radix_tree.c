...
radix_tree_get_result(result_t result)
{
    ...
    case SUCCEED:
        return "success";
    case FAILED:
        return "failed,perhaps this ip isn't in wblist";
    case NO_MEMORY:
...
void *
radix_tree_find_str(radix_tree_t *tree, const char *strip)
{
    struct in_addr myin;
...
static void
radix_tree_print_helper(radix_node_t *node, uint32_t ip, int mask)
{
    ...
    {
        mask++;
        radix_tree_print_helper(node->left, ip, mask);
        --mask;
    }
    ...
    if (node->right)
    {
        ...
        ip |= bit;
        mask++;
        radix_tree_print_helper(node->right, ip, mask);
        --mask;
    }
}

void
radix_tree_print(radix_tree_t *tree)
{
    radix_tree_print_helper(tree->root, 0, 0);
}

static void
radix_tree_print_helper_str(radix_node_t *node, char *ip, int len)
{
    if (!NODE_IS_EMPTY(node))
    {
        strncat(ip, "000000000000000000000000000000", 32-len);
        unsigned long temp = strtoul(ip, NULL, 2); 
        temp = htonl(temp);
        char ip_addr[16];
        inet_ntop(AF_INET, &temp, ip_addr, 16);
        printf("ip is %s/%d\n", ip_addr, len);
    ...
    {
        ip[len++] = '0';
        radix_tree_print_helper_str(node->left, ip, len);
        ip[--len] = '\0';
    }
    ...
    {
        ip[len++] = '1';
        radix_tree_print_helper_str(node->right, ip, len);
        ip[--len] = '\0';
    }
...
void
radix_tree_clear(radix_tree_t *tree)
{
    ip_node_delete(tree->root->left, tree->value_delete_func);
    ip_node_delete(tree->root->right, tree->value_delete_func);
    tree->root->right = NULL;
    tree->root->left = NULL;
    tree->root->parent = NULL;
    tree->root->value = ZIP_RADIX_NO_VALUE;
