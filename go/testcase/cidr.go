...
func main() {
	...
	fileFP, err := os.Open("ipquery.txt")
	if err != nil {
		fmt.Println("file not exists")
		return
	}
	defer fileFP.Close()
	resultFP, err := os.OpenFile("ipquery_range.txt", os.O_APPEND|os.O_RDWR|os.O_CREATE, 0666)
	if err != nil {
	...
	for {
		...
		minIp, maxIp := getCidrIpRange(line)
		fmt.Println("CIDR最小IP：", minIp, " CIDR最大IP：", maxIp)
		resultFP.WriteString(minIp + "|" + maxIp + "\n")
	}
